...
if use_folds:
    ...
    for fold in range(5):
        ...
        test_list_csv_folds.append(test_list_csv_fold)
    ...
    test_list_csv_folds = [test_list_csv]
...
for fold_idx, ref_obj_list in enumerate(ref_obj_list_folds):
    ...
    for ref_obj in ref_obj_list:
        ...
        test_dis_list = correlation_VP.get_testset_dis_list_from_ref(test_list_csv_folds[fold_idx], ref_obj)
        test_dis_names = {correlation_VP.normalize_name(name) for name in test_dis_list}
        distorted_obj_list = [
            name for name in distorted_obj_list
            if correlation_VP.normalize_name(name) in test_dis_names
        ]
